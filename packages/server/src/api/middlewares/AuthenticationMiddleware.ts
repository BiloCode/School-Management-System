...
export const AuthenticationUserMiddleware = async (
  ...
  const auth = new AuthenticationUser(new ComparePasswordWithBcrypt());
  const user_id_response = await auth.verify(dni, password);
  ...
  const token_generated = new CreateToken().create(
    user_id_response as string,
    user_type
  );
